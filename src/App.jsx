...
function Guild({hud,accept,claim,createParty,joinParty,leaveParty}){
  ...
  return <div className="guild-layout">
    <aside className="guild-rank-card"><small>RANK DA GUILDA</small><strong>{current.id}</strong><h3>{current.name}</h3><p>Nível de Guilda {currentIndex+1} • {hud.guildPoints||0} XP da Guilda</p>{next&&<><Bar value={Math.min(100,(hud.guildPoints||0)/Math.max(1,hud.guildNextRequirement||1)*100)} cls="guildbar"/><em>Próximo: {next.id} • Nv.{next.minLevel}+ • {hud.guildNextRequirement||0} XP</em></>}<div className="rank-strip">{GUILD_RANKS.map((r,i)=><span key={r.id} className={i===currentIndex?'active':i<currentIndex?'done':''}>{r.id}</span>)}</div></aside>
