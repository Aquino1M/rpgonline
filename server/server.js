...
const roomDefault='asterra-global'
const VERSION='0.9.9'
...
function storeProfile(id,name,game,updatedAt=Date.now(),lastLobby=''){let serialized='';try{serialized=JSON.stringify(game||{})}catch{return false};if(Buffer.byteLength(serialized,'utf8')>MAX_PROFILE_BYTES)return false;const old=database.players[id]||{};const lobby=roomDefault;database.players[id]={...old,id,name:sanitizeName(name||old.name),lastLobby:lobby,updatedAt:safeNumber(updatedAt,Date.now(),0,Date.now()+60000),lastSeen:Date.now(),game:JSON.parse(serialized)};scheduleSave();return true}
...
function joinRoom(ws,room){const old=rooms.get(ws.room);old?.delete(ws);if(old&&!old.size)rooms.delete(ws.room);ws.room=roomDefault;if(ws.id&&database.players[ws.id]){database.players[ws.id].lastLobby=ws.room;database.players[ws.id].lastSeen=Date.now();scheduleSave()}if(!rooms.has(ws.room))rooms.set(ws.room,new Set());rooms.get(ws.room).add(ws);send(ws,{type:'welcome',id:ws.id,room:ws.room,players:[...rooms.get(ws.room)].filter(x=>x!==ws&&x.player).map(x=>publicPlayer(x.player))});broadcast(rooms.get(ws.room),{type:'join',player:publicPlayer(ws.player)},ws)}
